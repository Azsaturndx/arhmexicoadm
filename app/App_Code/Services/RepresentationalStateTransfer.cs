        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                	...
                	sb.AppendFormat(string.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/arhmexico/_T" +
                ...
                sb.AppendFormat("var __targetFramework=\'4.6\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'arhmexicoa" +
                        "dm|{0}\';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            if (!(string.IsNullOrEmpty(callback)))
            	...
            	if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("arhmexico=typeof arhmexico==\'undefined\'?{{}}:arhmexico;arhmexico.{0}=", instance);
                }
        }
